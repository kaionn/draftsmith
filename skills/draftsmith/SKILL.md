---
name: draftsmith
description: 設計ファーストのインナーループを 1 タスク分回す。自然言語の要件（または Plans.md の 1 タスク行）を受け取り、要件正規化 → designer 設計 → 監査 3 層 → implementer 実装 → reviewer-light「指摘なし」まで自走する。「設計から実装して」「draftsmith で」「設計ファーストで進めて」で発火。--gated で要件確定・設計確定を人間確認にする。
user-invocable: true
---
