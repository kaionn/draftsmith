...
- 依存のない Read / Grep / Glob / Bash は **同一メッセージでまとめて発行**する。値の受け渡しが
  ある時だけ分割する。diff に含まれる全ファイルの現物は最初の 1 turn でまとめて Read する
- 対象ファイルの現物は Bash の `cat` / `sed -n` ではなく Read で開く
- 要件書の非機能要件へ転記済みの規範ファイル（`AI_CONTEXT.md`・`CLAUDE.md`・lint 設定等。
  非機能要件の「転記元: path（節名）」が示す）は開かない。観点 6（規約準拠）は渡された
  非機能要件を正として判定し、転記が不足していると判断した時だけその節を範囲指定で読む
- 調査 turn の目安は 8。超えたら現時点の結論で return し、未確認の観点を「懸念（判定保留）」へ書く
